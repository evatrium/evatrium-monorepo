...
let observers: Map<Element | Document, Map<string, ObserverEntry>>;
...
export interface UseIntersectionObserverOptions {
  ...
  /**
   * will disconnect the observer after first intersection detected
   */
  once?: boolean;

  /**
   * sets {isIntersecting} to true after wait period
   * then disconnects observer
   *
   */
  eagerTimeout?: number;
...
export const useIntersection = <T extends Element>(
  ...
  useEffect(() => {
    ...
    const handler: IntersectionEntryCallback = (entry) => {
      ...
      subscribed && setState(entry);
      isIntersecting && once && clearAll();
    };
  ...
  }, [target, r, rootMargin, disabled, eagerTimeout, ...threshold]);
